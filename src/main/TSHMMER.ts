...
export class TSHMMER {
  ...
  async run(): Promise<void> {
    ...
    switch (args.mode) {
      ...
      case Mode.Cleanup:
        const cleaned: string[] = Simplifier.cleanFile(await this.readFullFa(args));
      ...
      case Mode.UniqueFilter:
        console.warn("WARNING: This filter is not fully implemented and may remove sequences that are not duplicates if the sequence itself is identical but the name is not.");
        const file: string[] = await this.readFullFa(args);
        const unique: string[] = this.dataHandler.filterNonUniques(file);
        const filteredUnique: string[] = this.dataHandler.filterIncorrectLines(unique, args.readLength)
